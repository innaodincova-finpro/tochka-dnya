...
(async()=>{
 ...
 const handlers={};let shown,clicked;let windows=[];
 vm.runInNewContext(fs.readFileSync('sw.js','utf8'),{self:{addEventListener:(k,f)=>handlers[k]=f,registration:{scope:'https://test.invalid/app/',showNotification:async(title,options)=>{shown={title,...options};}},clients:{matchAll:async()=>windows,openWindow:async u=>{clicked=u;}}},URL});
 let promise;handlers.push({data:{json:()=>({title:'Встреча',body:'В 12:00',url:'https://evil.test'})},waitUntil:p=>promise=p});await promise;assert.equal(shown.title,'Встреча');handlers.notificationclick({notification:{close(){}},waitUntil:p=>promise=p});await promise;assert.equal(clicked,'https://test.invalid/app/index.html');
 windows=[{url:'https://test.invalid/app/reestr.html',focus:()=>{throw Error('Registry must not receive reminder focus');}}];
 handlers.notificationclick({notification:{close(){}},waitUntil:p=>promise=p});await promise;assert.equal(clicked,'https://test.invalid/app/index.html');
 let focused=false;windows.push({url:'https://test.invalid/app/index.html',focus:()=>{focused=true;}});
 handlers.notificationclick({notification:{close(){}},waitUntil:p=>promise=p});await promise;assert(focused);
