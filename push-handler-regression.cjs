...
(async()=>{
 ...
 let handler,providerCalls=0,providerStatus=201,reads=0,move=false,allowed=true;
 ...
 vm.runInNewContext(stripTypeScriptTypes(source),{...schedule,webpush,URL,Response,Request,AbortSignal,Uint8Array,Intl,Date:clock,Deno:{env:{get:k=>k==='SUPABASE_URL'?'https://test.invalid':'service-secret'},serve:f=>handler=f},fetch:async(url,opts={})=>{
  ...
  if(path==='tochka_members')return Response.json(allowed?[{user_id:uid}]:[]);
 ...
 allowed=false;assert.equal((await handler(req({action:'key'},{Authorization:'Bearer user'}))).status,403);allowed=true;
