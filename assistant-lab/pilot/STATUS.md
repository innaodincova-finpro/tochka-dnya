...
## Consent and deployment update
Owner explicitly approved forwarding new bot messages to DeepSeek on 2026-09-12. Receiver version 1 deployed successfully with custom secret authentication; live request without secret returned 401. Control version 3 deployed with JWT required; temporary rollout gate removed after approved receiver deployment. Owner activation UI published at /tochka-assistant/pilot.html; copied under assistant-link. Local jsdom checks verified login, password clearing, enable success and rejected-key UI. No actual owner activation/model response has been verified yet. Activation checks DeepSeek API balance/key and Telegram bot, then registers the webhook. User must sign in and enable; no secret or user session was extracted to bypass this step.
