...
Tests: node receiver-test.mjs; node sql-test.mjs (PGlite, synthetic DB). Before publishing apply direct-save.sql and inspect advisors. The main-branch deployment workflow assembles all runtime *.mjs plus index.ts into `supabase/functions/tochka-assistant-receiver`, injects the Git commit SHA, deploys the function, and verifies that exact SHA through its read-only health response. A green deployment therefore confirms the Telegram receiver version, not only the other Edge Functions. Keep original v4 as rollback source from prior git history. Reverting receiver disables direct buttons; committed records remain ordinary records. Do not restore all user data automatically.
